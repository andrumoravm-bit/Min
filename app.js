...
// =============================================
// URL DE GOOGLE SHEETS (CAMBIAR SI ES NECESARIO)
// =============================================
...
let dataSource = 'local'; // 'sheets' o 'local'
...
function parseCSV(csv) {
  ...
  for (let i = 1; i < lines.length; i++) { // skip header
    ...
    if (values.length >= 11 && !isNaN(values[0])) {
      data.push(values);
    }
  }
  return data;
...
async function loadData() {
  ...
  try {
    ...
    if (parsed.length < 5) throw new Error('Datos insuficientes');
    
    processData(parsed);
    dataSource = 'sheets';
    statusEl.textContent = '🟢 Datos en vivo desde Google Sheets — Última carga: ' + new Date().toLocaleString('es-VE');
    ...
    dataSource = 'local';
    statusEl.textContent = '🟡 Usando datos locales de respaldo — Google Sheets no disponible';
    statusEl.style.color = '#f59e0b';
  }
...
function buildKPIs() {
  ...
  MINERALS.forEach(m => {
    ...
    card.innerHTML = `
      ...
      <div class="kpi-change ${up ? 'up' : 'down'}">${up ? '▲' : '▼'} ${Math.abs(ch)}% vs ${prevYear}</div>
    `;
    ...
    options: {
      ...
      scales: {
        x: { grid: { color: tc.gridColor }, ticks: { color: tc.tickColor, font: { size: 11 }, autoSkip: true, maxRotation: 0,
          callback: function(value, index) { const y = years[index]; if (y === years[0] || y === years[years.length-1] || y % 5 === 0) return y; return ''; }
  ...
  MINERALS.forEach((m, idx) => {
    ...
    card.innerHTML = `
      <div class="chart-header">
        <div><div class="mineral-name"><span class="dot" style="background:${m.color}"></span>${m.key}</div><div class="mineral-unit">${m.unit} · ${years[0]}–${lastYear}</div></div>
        <div class="mineral-stats">
          ...
          <div class="stat"><div class="stat-label">${lastYear}</div><div class="stat-val" style="color:#16a34a">${fmt(current)}</div></div>
        </div>
      </div>
...
// =============================================
// INIT — Cargar datos al abrir la página
// =============================================
loadData();
